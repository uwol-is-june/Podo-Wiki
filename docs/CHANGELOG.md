...
- [TASK-081] 흰 배경 로고가 다크모드에서 '흰 네모'로 보이던 문제 — 웹 먼저 적용 (2026-08-07)
  ...
  - 로고 원본을 건드리는 대신 **받침(plate)을 깔아 의도된 타일로 보이게** 하는 방향. 흰 배경을 투명으로 지우는 안은 반대로 검은 글자 로고가 다크에서 안 보이게 되고, 자동 반전은 브랜드 색을 훼손해서 제외
  - `--wiki-logo-plate` 토큰 추가 — 라이트는 카드와 같은 `#ffffff`(눈에 안 띔), 다크는 라이트모드 배경색 `#f7f5fc`. 로고를 이 받침 위에 올리고 안쪽 여백을 줌(홈 88px 칸에 `p-2`, `/admin` 40px 썸네일에 `p-1`). TASK-078의 `trim()`이 바깥 여백을 잘라내 그림이 칸을 꽉 채우기 때문에 여백은 표시 단에서 줘야 함
  - 적용: 홈 단체 그리드(`src/app/page.tsx`), `/admin` 단체 표(`AdminTroupeTable.tsx`)
  - **앱은 아직** — 웹에서 실제로 보고 값을 확정한 뒤 옮기려고 분리했다 (TASK_M)
...
- [TASK-078] 단체마다 홈 로고 크기가 들쭉날쭉하던 문제 (2026-08-04)
  ...
  - `object-cover` 로는 안쪽 여백을 보정할 수 없고, 오히려 가로로 긴 광운 로고의 좌우가 잘리고 있었음
  - **업로드 시점에 sharp 로 테두리 여백을 잘라내고**(모서리 픽셀과 같은 색인 영역 제거) 한 변 320px 이하로 줄여 저장. 표시는 `object-contain` 으로 바꿔 잘림 없이 칸을 채우게 함. SVG 는 벡터 그대로 두고, 전부 단색이라 trim 이 실패하는 경우엔 원본을 그대로 올림
  - 검증: 실제 두 로고로 파이프라인을 돌려 시네씨아 1400×1400 → 320×240, 광운 1216×864 → 320×305 로 여백만 잘리고 그림은 온전한 것을 눈으로 확인. `sharp` 는 Next 기본 `serverExternalPackages` 에 포함돼 별도 설정 불필요
  - **이미 올라간 로고는 자동으로 고쳐지지 않음** — 관리자 수정 폼에서 같은 파일을 다시 올리면 새 파이프라인을 타서 정리됨
