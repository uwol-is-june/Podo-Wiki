...
| 플랫폼  | 스토어 버전 | 상태             |
| ------- | ----------- | ---------------- |
| iOS     | 1.1.1       | ⏳ ASC 업로드 완료, **심사 제출 대기** (8/5) |
| Android | 1.1.1       | ⏳ AAB 빌드 완료, **Play Console 업로드 대기** (8/5) |
| ~~iOS~~ | ~~1.1.0~~   | 출시됨 (8/3) — 현재 스토어에 걸린 버전 |
| ~~Android~~ | ~~1.1.0~~ | 출시됨 (8/3) — 현재 스토어에 걸린 버전 |

- **1.1.1 내용**: 1.1.0 출시 뒤 쌓인 앱 변경을 반영하는 유지보수 릴리스. iOS build 6 / Android vc 8
  ...
  - ⚠️ 시네씨아 문서는 DB에서 `content: ""` 상태였음 — 출시 전 내용을 채울 것. 안 그러면 새 단체 카드가 빈 화면으로 열림
...
| 버전  | 플랫폼  | 제출일     | 내용                                       |
| ----- | ------- | ---------- | ------------------------------------------ |
| 1.1.1 | 양대    | 2026-08-05 | 단체 목록 DB 전환(T-075·076) + 북마크 저장 경합(T-069) + 아이콘 통일(T-068) + 목차 이동(T-070) + FAB 깜빡임(T-071). iOS build 6 / Android vc 8 |
