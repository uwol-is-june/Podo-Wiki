...
- [ ] [TASK-057] (S) 안드로이드 릴리스 빌드에 R8 mapping 파일 포함 → Play Console 업로드 시 "가독화 파일 없음" 경고. mapping이 없으면 Android vitals 크래시 스택이 난독화된 채로 보여 디버깅 어려움 (TASK-056 때 실제로 겪은 문제). `mobile/android`는 CNG(gitignore)라 gradle 직접 수정 불가 — expo config plugin 또는 `expo-build-properties`로 `android.enableProguardInReleaseBuilds` 관련 설정 확인 후, AAB에 mapping 자동 포함되는지 다음 빌드에서 검증. 참고: AAB 업로드 후 Play Console → App Bundle 탐색기에서 수동 업로드도 가능.
